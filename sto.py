...
def update_table():
    try:
        ...
        if input_field.get() not in total_list_stock:
            ...
            collet_table.insert('',tk.END,values=(stock['info']['code'],stock['info']['name'],nowprice,stock['realtime']['accumulate_trade_volume']))
        else:
...
def touch(event):#當按下收藏中的股票時
    ...
    if col2 != "代碼":
        data = twstock.realtime.get(col2) #抓取股票資料
        print(data)
        get_data(data)
# 按下搜尋
def search():
...
#重新整理
def stock_re_button():
    x = collet_table.get_children() #取得表格內所有欄位
    print(x)
    for i in x:
...
def chart():
    ...
    fig,ax = mpf.plot(df, type='candlestick',volume=True,style=s,mav=(5,20),returnfig=True,block=False  )
...
def information():
    ...
    infolist= sp.find_all('span',class_='As(st) Bxz(bb) Pstart(12px) Py(8px) Bgc($c-gray-hair) C($c-primary-text) Flx(n) W(104px) W(120px)--mobile W(152px)--wide Miw(u) Pend(12px) Mend(0)')
    print(infolist)
    ...
    columns = ["項目","內容"]
    table = tk.ttk.Treeview(frame2,height=28,show="headings",columns=columns)
    ...
    for i in company_info:
        if i[0] == '主要經營業務':
            ...
            table.insert('','end',values=(i[0],tampinfo[0])) #插入資料
            for i in range(1,len(tampinfo)):
                table.insert('','end',values=("",tampinfo[i])) #插入資料
        else:
            table.insert('','end',values=(i[0],i[1]))#插入資料
...
win1 = tk.Tk()#股票資訊
win1.title('個股資訊')
win1.geometry('900x700+800+150') #視窗尺寸
show_switch = 0
f1 = tk.Frame(win1)
f1.pack()
...
total_list_stock = []
try:
    f = open('collect.txt') #把檔案內容讀入
    total_list_stock = list(f.read().split())
    f.close()
except:
    pass
print(total_list_stock)
frame33 = tk.Frame(win2)
frame33.grid(row=2,column=0,columnspan=4)
columns = ["代碼","名稱","成交價","成交量"]

collet_table = ttk.Treeview(frame33,height=15,show="headings",columns=columns)
